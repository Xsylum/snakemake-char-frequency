...
cmdline = f"""sbatch -N {cluster_nodes} -n {cluster_ntasks} -p {cluster_partition} \
                     ...
                     --mail-type={cluster_mail_type} --mail-user={cluster_mail_user} \
                     {resources_mem} {clusters} {qos} {jobscript}"""
print(cmdline)
logging.info(f"rule: {rulename}")
logging.info(f"job_properties['resources']: {job_properties['resources']}")
